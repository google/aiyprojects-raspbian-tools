...
from imgtools import SECTOR_BYTES, LoopDev, Mount, get_partition_info, resize2fs, resize_part
...
def do_resize(args):
  ...
  with LoopDev(args, args.image) as disk_dev:
    ...
    start_sector = get_partition_info(args, disk_dev)[args.root_partition_number].start
    ...
    with LoopDev(args, args.image, offset=start_bytes) as root_dev:
      ...
      try:
        ...
        print('WARNING: Failed to zero free blocks, compressed image will be larger.', file=sys.stderr)
    ...
    resize_part(args, disk_dev, args.root_partition_number, start_sector, new_end_sector)
    ...
    if written_end != new_end_sector:
      ...
      print('  expected %d, read %d' % (new_end_sector, written_end), file=sys.stderr)
      sys.exit(1)
...
def set_expand(args, expand):
  ...
  with LoopDev(args, args.image) as disk_dev:
    ...
    boot_start_bytes = paritition_info[args.boot_partition_number].start * SECTOR_BYTES
    root_start_bytes = paritition_info[args.root_partition_number].start * SECTOR_BYTES

    with Mount(args, args.image, offset=boot_start_bytes) as boot_mnt, \
        Mount(args, args.image, offset=root_start_bytes) as root_mnt:
          try:
            set_expand_boot(args, boot_mnt, expand)
            set_expand_root(args, root_mnt, expand)
          except PermissionError:
            print('ERROR: Script must be run with sudo to set [no-]expand', file=sys.stderr)
            sys.exit(1)
